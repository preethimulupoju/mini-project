class Account:
    ...
    #Define a __inti__ constructore with attributes shared by all accounts
    def __init__(self, acc_nbr, opening_balance):
        self.acc_nbr = acc_nbr
        self.balance = opening_balance

    #Define a __str__ method to print attributes for accounts
    def __str__(self):
        return f'{self.acc_nbr}\nBalance: ${self.balance:.2f}\n'
    
    #Define a universal deposite method to accept deposit
    def deposit(self, dep_amnt):
        self.balance += dep_amnt

    #Define a universal withdrawl method to handle withdrawls
...
#Define a Checking specific class that inherits from base class Account, adds checking specific traits 
class Checking(Account):

    #Define init method which runs the base class init
    def __init__(self, acc_nbr, opening_deposit):
        super().__init__(acc_nbr, opening_deposit)

    #Define the __str__ method which return the bank statement as a string
    def __str__(self):
        return f'Checking Account: #{Account.__str__(self)}'

#Define a Saving class that inherits from base class Account 
